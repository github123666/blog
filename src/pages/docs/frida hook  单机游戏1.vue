...
![version](./static/version.png)
## 启动 ! ! !
1. 打开安卓模拟器目录顶部输入cmd并回车
![cmd](./static/openCmd.png)  
2. 执行以下命令将下好的frida-server包导入到模拟器中
> adb push frida包的路径 /data/local/tmp 或/data/local/temp 
> 
3. 进入shell,cd到刚刚放frida-server的目录下
> adb shell \  
> cd /data/local/tmp  \  
...
>
5. 打开pycharm 输入frida-ps -u 命令行会打印很多进程信息(基本配置已完成)
## 反编译
直接无脑使用工具 
